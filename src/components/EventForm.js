...
const EventForm = (props) => {
  ...
  const handleSubmit = () => {//HOF    
    const event = {
      dtstart: new Date(document.getElementById("dtstart").value).toString(),
      dtend: new Date(document.getElementById("dtend").value).toString(),
    ...
    //const working_minutes = 600;
    const duration_milliseconds = moment(event.dtend).diff(event.dtstart);
    const duration_minutes = Math.floor((duration_milliseconds / 1000 / 60) << 0);
    if (duration_minutes > 0 ) {
      console.log('setting up your time...')
      props.onFormSubmit(Object.assign({}, event, { duration: duration_minutes }));
    } else {
      window.alert('Invalid Time Request, check if its below the working minutes and try again...')
    }
